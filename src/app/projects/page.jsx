...
export default function Projects() {
  ...
  // ✅ Bug 2 fixed: `filtered` defined at component level so JSX can access it
                  ...
                  {repo.topics?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {repo.topics.slice(0, 3).map((topic) => (
                        <span
                          key={topic}
                          className="text-[11px] font-medium text-blue-300/70 bg-blue-500/[0.08] border border-blue-500/15 rounded-full px-2.5 py-0.5"
                        >
                          {topic}
                        </span>
                      ))}
